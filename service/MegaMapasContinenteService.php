...
class MegaMapasContinenteService
{
    private $validator;
    private $view;
    private $routes;
    ...
    public function __construct()  
    {
        ...
        $this->routes = new MegaMapasContinenteRoutes();
        $this->view = new MegaMapasPrincipalView();
        $this->repository = new MegaMapasContinenteRepository();
    }
 
    public function buscarContinente($rota)
    {
        $resultado = [];
        ...
        try { 
            ...
            if ($this->validator->validarClasseMetodo($this->routes, 'consultarRotas'))
            {
                ...
                //repository
                if ($this->validator->validarClasseMetodo($this->repository, 'consultarContinente')) 
                {
                    $repository = $this->repository->consultarContinente($rota);
                   ...
                   $this->mensagem[] = MegaMapasPrincipalException::inexistente(__METHOD__, 'consultarContinente.inexistente');
                }
