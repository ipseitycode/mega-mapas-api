...
class MegaMapasRuaService
{
    ...
    public function __construct()  
    {
        ...
        $this->routes = new MegaMapasRuaRoutes();
        $this->view = new MegaMapasPrincipalView();
        $this->repository = new MegaMapasRuaRepository();
    }
 
    public function buscarRua($rota)
    {
        $resultado = [];
        ...
        try { 
            ...
            if ($this->validator->validarClasseMetodo($this->routes, 'consultarRotas'))
            {
                ...
                //repository
                if ($this->validator->validarClasseMetodo($this->repository, 'consultarRua')) 
                {
                    $repository = $this->repository->consultarRua($rota);
                   ...
                   $this->mensagem[] = MegaMapasPrincipalException::inexistente(__METHOD__, 'consultarRua.inexistente');
                }
