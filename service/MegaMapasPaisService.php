...
class MegaMapasPaisService
{
    ...
    public function __construct()  
    {
        ...
        $this->routes = new MegaMapasPaisRoutes();
        $this->view = new MegaMapasPrincipalView();
        $this->repository = new MegaMapasPaisRepository();
    }
 
    public function buscarPais($rota)
    {
        $resultado = [];
        ...
        try { 
            ...
            if ($this->validator->validarClasseMetodo($this->routes, 'consultarRotas'))
            {
                ...
                //repository
                if ($this->validator->validarClasseMetodo($this->repository, 'consultarPais')) 
                {
                    $repository = $this->repository->consultarPais($rota);
                   ...
                   $this->mensagem[] = MegaMapasPrincipalException::inexistente(__METHOD__, 'consultarPais.inexistente');
                }
