...
class MegaMapasEstadoService
{
    ...
    public function __construct()  
    {
        ...
        $this->routes = new MegaMapasEstadoRoutes();
        $this->view = new MegaMapasPrincipalView();
        $this->repository = new MegaMapasEstadoRepository();
    }
 
    public function buscarEstado($rota)
    {
        $resultado = [];
        ...
        try { 
            ...
            if ($this->validator->validarClasseMetodo($this->routes, 'consultarRotas'))
            {
                ...
                //repository
                if ($this->validator->validarClasseMetodo($this->repository, 'consultarEstado')) 
                {
                    $repository = $this->repository->consultarEstado($rota);
                   ...
                   $this->mensagem[] = MegaMapasPrincipalException::inexistente(__METHOD__, 'consultarEstado.inexistente');
                }
